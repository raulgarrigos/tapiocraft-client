      ...
      <div className="grid gap-4 grid-cols-1">
        ...
        {allOrders.map((order) => {
          return (
            <div
              key={order._id}
              className="bg-white rounded-lg shadow-md w-2/4 p-4 transition duration-300 ease-in-out transform hover:scale-105 hover:border border-pistachoGreen mx-auto"
            >
              <Link to={`/profile/${params.userId}/orders/${order._id}`}>
                <p className="font-bold">Pedido Nº:</p>
                <p className="text-pistachoGreen"> {order._id}</p>
              </Link>
            </div>
          );
        })}
